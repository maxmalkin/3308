					...
					<div className="mt-9 grid items-center gap-10 rounded-2xl bg-ink px-10 py-14 text-paper md:grid-cols-[1.4fr_auto]">
						<div>
							...
							<h2 className="mt-2 max-w-[24ch] text-[clamp(26px,2.8vw,40px)] font-medium leading-[1.05] tracking-[-0.03em]">
								Build a watch life you'll actually{" "}
								<em>remember</em>.
							</h2>
						</div>
						<Link
	...
	return (
		<>
			...
			<p className="m-0 max-w-[56ch] text-sm leading-[1.55] text-ink-2">
				Add anything with a tap. Reorder it, or let PillarBoxd pick one
				on indecisive nights. When you press play, it moves to your log.
			</p>
...
function LogPreview() {
	...
	return (
		<>
			...
			<p className="m-0 max-w-[56ch] text-sm leading-[1.55] text-ink-2">
				Rate a show and it's filed away. Your log is the running record
				of every episode and season you've finished.
			</p>
			<div className="flex flex-col">
				{rows.map((r) => (
					<div
...
function RecsPreview() {
	...
	return (
		<>
			...
			<p className="m-0 max-w-[56ch] text-sm leading-[1.55] text-ink-2">
				Based on the shows you've rated highly — not on what a studio
				paid us to surface.
			</p>
...
function TastePreview() {
	...
	return (
		<>
			...
			<p className="m-0 max-w-[56ch] text-sm leading-[1.55] text-ink-2">
				Genres you lean on, how much you've tracked, and what's been
...
function PosterWall() {
	const layout = [
		{
			stub: STUBS[0],
			cls: "w-[180px] left-0 top-[10px] -rotate-[5deg] z-[2]",
		},
		{
			stub: STUBS[1],
	...
	return (
		<div className="absolute inset-x-0 top-10 bottom-0">
			<span
				...
				style={{
					background:
						"color-mix(in oklab, var(--mustard) 70%, transparent)",
					borderColor:
						"color-mix(in oklab, var(--mustard) 80%, #000 10%)",
				}}
			/>
			<span
